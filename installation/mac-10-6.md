...
GNU nano text editor will open. Paste (cmd-v) `export SELENIUM_SERVER_JAR=/usr/bin/selenium-server-standalone-2.5.0.jar`. Exit GNU nano and save the file with *control+x*, press *y* when it asks `Save modified buffer (ANSWERING "No" WILL DESTROY CHANGES) ?` and press *Enter* when it displays `Save modified buffer (ANSWERING "No" WILL DESTROY CHANGES) ?`.
