...
Sometimes `id`s on a page are automatically generated by the framework developers use, so `id`s are different every time a page is opened. Usually the portion of `id` stays the same. For example, it changes from `user007` to `user42` to `user123`. It is obvious that `user` part of the `id` is the same every time, so you can use that.
